...
explizit _nicht_ nach `+/etc/apt/trusted.gpg.d/+` kopieren, weil dann
alle Pakete aus allen Quellen mit dem Elastic-Key signiert sein
duerften. Dazu legen wir den keyring nach `+/usr/local/share/keyrings/+`
und verwenden diesen Pfad im sources file:
...
URL selbst an: `+https://artifacts.elastic.co/packages/8.x/apt+`.
Nachdem wir die package-Struktur eines Debian repos kennen, bedeutet
dies, dass wir ein `+Packages.gz+` fuer unsere Plattform finden unter
`+<REPO-URL>/dists/stable/main/binary-amd64/Packages.gz+`. Und genau
damit probieren wir es:
