...
Wenn Platten akputt gehen, muss man sie auswechseln. Wenn sie an einem
RAID-Controller hängen, kümmert der sich drum, wenn aber den Controller
im HBA-Mode betreiben und die Platten als JBOD, dann müßen wir uns schon
sleber drum kümmern.
...
Am Server selber leuchtet die Fehler-LED von einer Platte, Box 3 Bay 2.
Dieser Controller ist im HBA mode, und exportiert nur die blockdevices,
keine logical disks (FIXME: nicht ganz richtig, mit der hpsa man page
erweitern). Welche Platte genau ist kaputt? Aus dem device name sehen
wir die WWN:
...
/dev/sdg ist unser Problemkind. Jetzt ermitteln wir die SCSI ID, damit
wir die Platte einer box/bay zuordnen koennen:
...
Mit dem sg_map Kommando aus den sg3-tools koennen wir die SCSI-ID zu
diesem Device ermitteln:
...
Unsere Platte hat also die SCSI-ID `+KSJ8SEWF+`. Wo haengt die am
Controller?:
...
Die Platte mit der SCSI-ID KSJ8SEWF befindet sich in Box 3, Bay 2. Weil
wir ordentlich sind nehmen wir sie die Partition im ZFS und im MD
offline. Zuerst der ZFS Pool::
...
Jetzt koennen wir die Platte abstecken. Das sehen wir anschliessend auch
in den kernel messages:
